...
function ReceiptModal({order,close}:{order:Row;close:()=>void}){const items=(order.items||[]).filter((x:any)=>!x.isBonus),bonuses=(order.items||[]).filter((x:any)=>x.isBonus);const pages:any[][]=[];const perPage=18;for(let i=0;i<items.length;i+=perPage)pages.push(items.slice(i,i+perPage));if(!pages.length)pages.push([]);const moneyText=(v:any)=>new Intl.NumberFormat("ru-RU").format(Number(v)||0)+" UZS";const download=()=>{const pageW=794,pageH=1123,rowH=34,headerH=210,bonusH=bonuses.length?150:0,totalH=Math.max(pageH,headerH+Math.max(1,items.length)*rowH+bonusH+90);const canvas=document.createElement("canvas");canvas.width=pageW;canvas.height=totalH;const c=canvas.getContext("2d")!;c.fillStyle="#fff";c.fillRect(0,0,pageW,totalH);c.fillStyle="#17202a";c.font="bold 22px Arial";c.fillText("ЧЕК · Заказ №"+order.orderNumber,36,44);c.font="12px Arial";c.fillText("Продавец: "+(order.creator||"—")+" · "+(order.creatorPhone||"—"),36,72);c.fillText("Клиент: "+(order.customer||"—")+" · "+(order.customerPhone||"—"),36,94);c.fillText("Адрес: "+(order.customerAddress||"—"),36,116);c.fillText("Дата: "+displayValue(order.createdAt),36,138);let y=180;c.font="bold 11px Arial";c.fillText("Товар",36,y);c.fillText("Цена/шт.",420,y);c.fillText("Кол-во",560,y);c.fillText("Сумма",650,y);y+=18;c.font="11px Arial";items.forEach((x:any)=>{c.fillText(String(x.product).slice(0,48),36,y);c.fillText(moneyText(x.unitPrice),420,y);c.fillText(String(x.quantity),560,y);c.fillText(moneyText(x.total),650,y);y+=rowH;});c.font="bold 16px Arial";c.fillText("Итого: "+moneyText(order.total),36,y+12);if(bonuses.length){y+=55;c.font="bold 13px Arial";c.fillText("Бонусы",36,y);y+=24;c.font="11px Arial";bonuses.forEach((x:any)=>{c.fillText(String(x.product),36,y);c.fillText(String(x.quantity)+" шт.",560,y);c.fillText("БЕСПЛАТНО",650,y);y+=rowH;});}const a=document.createElement("a");a.download="chek-"+order.orderNumber+".png";a.href=canvas.toDataURL("image/png");a.click();};return <div className="modal-backdrop receipt-backdrop"><div className="receipt-modal"><button className="modal-close" onClick={close}>×</button><div className="receipt-actions"><button className="button outline" onClick={download}>Скачать PNG</button><button className="button primary" onClick={close}>Закрыть</button></div><div className="receipt-pages">{pages.map((pageItems,pi)=><div className="receipt-page" key={pi}><div className="receipt-head"><b>ЧЕК · Заказ №{order.orderNumber}</b><span>Дата: {displayValue(order.createdAt)}</span><span>Менеджер: {order.creator||"—"} · {order.creatorPhone||"—"}</span><span>Клиент: {order.customer||"—"} · {order.customerPhone||"—"}</span><span>Адрес: {order.customerAddress||"—"}</span></div><table className="receipt-table"><thead><tr><th>Товар</th><th>Цена за 1 шт.</th><th>Количество</th><th>Сумма</th></tr></thead><tbody>{pageItems.map((x:any)=><tr key={x.id}><td>{x.product}</td><td>{moneyText(x.unitPrice)}</td><td>{x.quantity}</td><td>{moneyText(x.total)}</td></tr>)}</tbody></table>{pi===pages.length-1&&<><div className="receipt-grand-total">Итого: {moneyText(order.total)}</div>{bonuses.length>0&&<div className="receipt-bonuses"><h3>Бонусы</h3><table><thead><tr><th>Товар</th><th>Количество</th><th>Цена</th></tr></thead><tbody>{bonuses.map((x:any)=><tr key={x.id}><td>{x.product}</td><td>{x.quantity}</td><td>БЕСПЛАТНО</td></tr>)}</tbody></table></div>}</>}</div>)}</div></div></div>;}
