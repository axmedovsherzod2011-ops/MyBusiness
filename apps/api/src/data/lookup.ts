...
import { branches, customers, orders, products, warehouses } from "../db/schema.js";
...
async function companyUser(req: AuthenticatedRequest) {
  const u = await db.query.users.findFirst({ where: eq((await import("../db/schema.js")).users.authSubject, req.firebaseUser!.uid) });
  ...
  try {
    ...
    if (type === "customers") {
      return res.json(await db.select({ id: customers.id, code: customers.code, name: customers.name })
        .from(customers)
        .where(and(eq(customers.companyId, u.companyId), or(ilike(customers.id, like), ilike(customers.code, like), ilike(customers.name, like))))
    ...
    if (type === "products") {
      return res.json(await db.select({ id: products.id, sku: products.sku, barcode: products.barcode, name: products.name })
        .from(products)
        .where(and(eq(products.companyId, u.companyId), or(ilike(products.id, like), ilike(products.sku, like), ilike(products.barcode, like), ilike(products.name, like))))
    ...
    if (type === "branches") {
      return res.json(await db.select({ id: branches.id, code: branches.code, name: branches.name })
        .from(branches)
        .where(and(eq(branches.companyId, u.companyId), or(ilike(branches.id, like), ilike(branches.code, like), ilike(branches.name, like))))
    ...
    if (type === "warehouses") {
      return res.json(await db.select({ id: warehouses.id, code: warehouses.code, name: warehouses.name })
        .from(warehouses)
        .where(and(eq(warehouses.companyId, u.companyId), or(ilike(warehouses.id, like), ilike(warehouses.code, like), ilike(warehouses.name, like))))
    ...
    if (type === "orders") {
      return res.json(await db.select({ id: orders.id, orderNumber: orders.orderNumber, status: orders.status })
        .from(orders)
        .where(and(eq(orders.companyId, u.companyId), or(ilike(orders.id, like), sql`CAST(${orders.orderNumber} AS TEXT) ILIKE ${like}`)))
